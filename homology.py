...
class MatrixProcessor(object):
    def __init__(self, matrix_first, matrix_second, log_error=True):
        ...
        self._shift = 0  # shit is equal to the number of bases elements, which are not in ker(M1)
...
class ChainComplex(object):
    ...
    def add_factor(self, index, factor_generators):
        ...
        List of factor_generators whould be list of tuples of length dim (c_{index}). Each generator will be added as a column to the scond matrix for calculating homology group
        '''
